...
def prepare_dataset(config):
    train_matches = pd.read_csv(f"{config['data_folder']}train.csv") # len = 30172
    val_matches = pd.read_csv(f"{config['data_folder']}val_given.csv")
    test_matches = pd.read_csv(f"{config['data_folder']}test_given.csv")

    # encode shot type
    codes_type, uniques_type = pd.factorize(train_matches['type']) # len = 30172, 10
    ...
    train_dataset = BadmintonDataset(train_matches, config) # len = 2268
    # new for splitting training data
    """
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=config['batch_size'], shuffle=True)  #71
    #train_dataloader = DataLoader(train_dataset, batch_size=config['batch_size'], sampler=train_sampler) #67
    ...
    # new for splitting training data
    #test_dataloader = DataLoader(train_dataset, batch_size=config['batch_size'], sampler=test_sampler) 
    #print(len(test_dataloader)) # 5
