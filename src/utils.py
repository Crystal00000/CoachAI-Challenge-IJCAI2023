...
def draw_loss(record_total_loss, config):
    output_loss = lambda loss_type: f'{loss_type}: {record_total_loss[loss_type]}'
    output = '\n'.join([output_loss(l_type) for l_type in ['total', 'shot', 'area']])
    print(output)
    with open(f"{config['output_folder_name']}/loss.txt", 'w') as f:
      f.write(output)
